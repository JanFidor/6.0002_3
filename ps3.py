...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        self.width = width
        self.height = height
        self.default_dirt = dirt_amount
        self.tiles = {}
        self.count_cleaned = 0

        for i in range(int(self.width)):
            for ii in range(int(self.height)):
                self.tiles[(i, ii)] = self.default_dirt
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        y = int(pos.get_y())
        if self.tiles[(x, y)]:
            dirt_left = max(self.tiles[(x, y)] - capacity, 0)
            self.tiles[(x, y)] = dirt_left
            if not dirt_left:
    ...
    def is_position_in_room(self, pos):
        ...
        if pos.get_x() < 0 or pos.get_x() >= self.width:
            return False
        if pos.get_y() < 0 or pos.get_y() >= self.height:
            return False
        return True
...
class FaultyRobot(Robot):
    ...
    than simply cleaning the tile it
     moves to.
    """
    p = 0.15
